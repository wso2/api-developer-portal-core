...
const getUserSubscriptionsWithDetails = async (orgID, userID) => {
  try {
    ...
    logger.error(
      { error, orgID, userID },
      "getUserSubscriptionsWithDetails failed",
    );
...
const deleteOrganization = async (orgId) => {
  try {
    ...
    if (deletedRowsCount < 1) {
      throw Object.assign(
        new Sequelize.EmptyResultError("Organization not found"),
      );
    }
    return deletedRowsCount;
  } catch (error) {
...
const deleteOrgContent = async (orgId, viewName, fileName) => {
  ...
  try {
    ...
    if (deletedRowsCount < 1) {
      throw Object.assign(
        new Sequelize.EmptyResultError("Organization content not found"),
      );
    }
    return deletedRowsCount;
  } catch (error) {
...
const deleteApplication = async (orgID, appID, userID) => {
  try {
    ...
    if (deletedRowsCount < 1) {
      throw Object.assign(
        new Sequelize.EmptyResultError("Application not found"),
      );
    }
    return deletedRowsCount;
  } catch (error) {
    ...
    const subMapping = await SubscriptionMapping.create(
      {
        ...
        CHECKOUT_SESSION_ID: subscription.CHECKOUT_SESSION_ID || null,
      },
      { transaction: t },
    );
    return subMapping;
...
const getSubscriptions = async (orgID, appID, apiID) => {
  try {
    return await SubscriptionMapping.findAll({
      where: {
        ORG_ID: orgID,
        [Sequelize.Op.or]: [{ APP_ID: appID }, { API_ID: apiID }],
        // Exclude non-active subscriptions
        PAYMENT_STATUS: {
          [Sequelize.Op.notIn]: ["PENDING", "CANCELED", "PAYMENT_FAILED"],
        },
      },
    });
  } catch (error) {
  ...
  try {
    ...
    if (Object.keys(updateData).length === 0) {
      throw new Sequelize.ValidationError(
        "No billing fields provided for update",
      );
    }
 ...
 */
const findSubscriptionByUniqueKey = async (
  orgID,
  appID,
  apiID,
  policyID,
    ...
    return await SubscriptionMapping.findAll({
      where: {
        ...
        [Sequelize.Op.or]: [
          ...
          {
            PAYMENT_STATUS: {
              [Sequelize.Op.notIn]: ["PENDING", "PAYMENT_FAILED"],
            },
          },
        ],
      },
...
const deleteAppKeyMapping = async (orgID, appID, apiID, t) => {
  try {
    ...
    if (deletedRowsCount < 1 && apiID !== null) {
      throw Object.assign(
        new Sequelize.EmptyResultError("Application Key Mapping not found"),
      );
    }
    return deletedRowsCount;
  } catch (error) {
...
const createAppKeyMapping = async (appKeyMap, t) => {
  try {
    const appKeyMapping = await ApplicationKeyMapping.create(appKeyMap, {
      transaction: t,
    });
...
/**
 * Get subscription with meter ID for usage tracking (Bug 4.2)
 */
...
const getApplicationAPIMapping = async (
  orgID,
  appID,
  apiID,
  appRefID,
