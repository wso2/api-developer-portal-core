  ...
  try {
    await sequelize.transaction(
      ...
      async (t) => {
        ...
        const labels = await apiDao.createLabels(
          organization.ORG_ID,
        ...
        await adminDao.createProvider(
          organization.ORG_ID,
          { name: "WSO2", providerURL: config.controlPlane.url },
          t,
        );
        ...
        if (config.generateDefaultSubPolicies) {
          await apiDao.bulkCreateSubscriptionPolicies(
            orgId,
            constants.DEFAULT_SUBSCRIPTION_PLANS,
            t,
          );
        }
...
const updateIdentityProvider = async (req, res) => {
  ...
  try {
    ...
    const [updatedRows, updatedIDP] = await adminDao.updateIdentityProvider(
      orgId,
      idpData,
    );
    if (!updatedRows) {
...
const createOrgContent = async (req, res) => {
  ...
  try {
    ...
    const files = await util.readFilesInDirectory(
      extractPath,
      orgId,
      req.protocol,
      req.get("host"),
...
const updateOrgContent = async (req, res) => {
  ...
  try {
    ...
    for (const { filePath, fileName, fileContent, fileType } of files) {
      if (fileName != null && !fileName.startsWith(".")) {
        const organizationContent = await getOrgContent(
          orgId,
          viewName,
          fileType,
          fileName,
...
const deleteOrgContent = async (req, res) => {
  ...
  try {
    ...
    if (!req.query.fileName) {
      deletedRowsCount = await adminDao.deleteAllOrgContent(
        orgId,
        req.params.name,
      );
    } else {
      deletedRowsCount = await adminDao.deleteOrgContent(
        orgId,
        req.params.name,
        fileName,
      );
...
const deleteAllOrgContent = async (req, res) => {
  ...
  try {
    const deletedRowsCount = await adminDao.deleteAllOrgContent(
      orgId,
      req.params.name,
      fileName,
    );
...
const createProvider = async (req, res) => {
  ...
  const extraKeys = util.rejectExtraProperties(
    ["name", "providerURL"],
    payload,
  );
...
const getProvidetByName = async (orgID, name) => {
  ...
  if (providerData.length > 0) {
    ...
    for (const provider of providerData) {
      providerResponse[provider.dataValues.PROPERTY] =
        provider.dataValues.VALUE;
    }
  ...
  if (providers.length > 0) {
    for (const provider of providers) {
      ...
      for (const [key, value] of Object.entries(
        provider.dataValues.properties,
      )) {
...
const deleteProvider = async (req, res) => {
  ...
  try {
    ...
    if (req.query.property) {
      ...
      deletedRowsCount = await adminDao.deleteProviderProperty(
        orgId,
        property,
        providerName,
      );
...
const createDevPortalApplication = async (req, res) => {
  ...
  try {
    ...
    try {
      const application = await adminDao.createApplication(
        orgId,
        userID,
        applicationData,
      );
...
const updateDevPortalApplication = async (req, res) => {
  ...
  try {
    ...
    const [updatedRows, updatedApp] = await adminDao.updateApplication(
      orgId,
      appId,
      userId,
      applicationData,
...
const deleteDevPortalApplication = async (req, res) => {
  ...
  const userID = req[constants.USER_ID];
  try {
    const appDeleteResponse = await adminDao.deleteApplication(
      orgId,
      appId,
      userID,
    );
...
const createSubscription = async (req, res) => {
  ...
  try {
    ...
    await sequelize.transaction(
      ...
      async (t) => {
        try {
          sharedApp = await adminDao.getApplicationKeyMapping(
            orgID,
            req.body.applicationID,
            true,
          );
          nonSharedApp = await adminDao.getApplicationKeyMapping(
            orgID,
            req.body.applicationID,
            false,
          );
          ...
          if (sharedApp.length > 0) {
            ...
            await handleSubscribe(
              orgID,
              req.body.applicationID,
              sharedApp[0].dataValues.API_REF_ID,
              sharedApp[0].dataValues.SUBSCRIPTION_REF_ID,
              response,
              isShared,
              t,
            );
            ...
            await handleSubscribe(
              orgID,
              req.body.applicationID,
              nonSharedApp[0].dataValues.API_REF_ID,
              nonSharedApp[0].dataValues.SUBSCRIPTION_REF_ID,
              response,
              isShared,
              t,
            );
          ...
          trackSubscribeApi(
            {
              ...
              idpId: req.isAuthenticated()
                ? req[constants.USER_ID] || req.user.sub
                : undefined,
            },
            req,
          );
          ...
          try {
            if (error.statusCode && error.statusCode === 409) {
              const appRef =
                sharedApp.length > 0 ? sharedApp[0] : nonSharedApp[0];
              const response = await invokeApiRequest(
                req,
                "GET",
                `${controlPlaneUrl}/subscriptions?applicationId=${appRef.dataValues.CP_APP_REF}`,
                {},
                {},
              );
              ...
              for (const subscription of response.list) {
                if (subscription.apiId === req.body.apiReferenceID) {
                  logger.info(
                    "Subscription already exists in control plane, updating database",
                    {
                      orgId: req.params?.orgId,
                  ...
                  await handleSubscribe(
                    orgID,
                    req.body.applicationID,
                    appRef.dataValues.API_REF_ID,
                    appRef.dataValues.SUBSCRIPTION_REF_ID,
                    subscription,
                    sharedApp.length > 0 ? true : false,
                    t,
                  );
                  // Bug 3.1: Skip DP row creation if an ACTIVE subscription already exists
                  const existingSub2 =
                    await adminDao.findSubscriptionByUniqueKey(
                      orgID,
                      req.body.applicationID,
                      req.body.apiId,
                      req.body.policyId,
                  ...
                  if (
                    !existingSub2 ||
                    existingSub2.PAYMENT_STATUS !== "ACTIVE"
                  ) {
                    await adminDao.createSubscription(orgID, req.body, t);
                  }
                  return res
...
const updateSubscription = async (req, res) => {
  ...
  try {
    await sequelize.transaction(
      ...
      async (t) => {
        try {
          let app = await adminDao.getApplicationKeyMapping(
            orgID,
            req.body.applicationID,
            true,
          );
          ...
          if (app.length === 0) {
            app = await adminDao.getApplicationKeyMapping(
              orgID,
              req.body.applicationID,
              false,
            );
          ...
          if (app.length > 0) {
            ...
            let appAPIMapping;
            appAPIMapping = await adminDao.getApplicationAPIMapping(
              orgID,
              req.body.applicationID,
              req.body.apiReferenceID,
              cpAppRef,
              true,
            );
            ...
            if (appAPIMapping.length > 0) {
              ...
              const subscruibedPolicy = await apiDao.getSubscriptionPolicy(
                req.body.policyId,
                orgID,
              );
              ...
              const subscriptionRefID =
                appAPIMapping[0].dataValues.SUBSCRIPTION_REF_ID;
              ...
              const response = await invokeApiRequest(
                req,
                "PUT",
                `${controlPlaneUrl}/subscriptions/${subscriptionRefID}`,
                {},
                {
                  apiId: req.body.apiReferenceID,
          ...
          return res
            .status(201)
            .json({ message: "Updated subscription successfully" });
        } catch (error) {
          logger.error("Error occurred while subscribing to API", {
            error: error.message,
            orgId: req.params?.orgId,
  ...
  try {
    ...
    await sequelize.transaction(
      ...
      async (t) => {
        ...
        const subDeleteResponse = await adminDao.deleteSubscription(
          orgID,
          subID,
          t,
        );
        if (subDeleteResponse === 0) {
          ...
          const subIDList = await adminDao.getAPISubscriptionReference(
            orgID,
            subscription.dataValues.APP_ID,
            subscription.dataValues.REFERENCE_ID,
            t,
          );
          ...
          for (const subscription of subIDList) {
            ...
            await invokeApiRequest(
              req,
              "DELETE",
              `${controlPlaneUrl}/subscriptions/${subscriptionID}`,
              {},
              {},
            );
            await adminDao.deleteAppKeyMapping(
              orgID,
              subDeleteResponse.APP_ID,
              subscriptionID,
              t,
            );
          }
  ...
  try {
    ...
    await sequelize.transaction(
      ...
      async (t) => {
        ...
        const appIDResponse = await adminDao.getApplicationID(
          orgID,
          userID,
          applicationName,
        );
        ...
        const sharedToken = await adminDao.getApplicationKeyMapping(
          orgID,
          appID,
          true,
        );
        const nonSharedToken = await adminDao.getApplicationKeyMapping(
          orgID,
          appID,
          false,
        );
        ...
        if (sharedToken.length !== 0) {
          ...
          const cpAppCreationResponse = await createCPApplication(
            req,
            cpApplicationName,
          );
        ...
        for (const sub of subAPIs) {
          ...
          const policyDetails = await apiDao.getSubscriptionPolicy(
            api.policyID,
            orgID,
            t,
          );
          ...
          const subJunction =
            sub.DP_APPLICATIONs?.[0]?.DP_API_SUBSCRIPTION?.dataValues;
          let billingData = null;
          if (
            subJunction?.BILLING_CUSTOMER_ID &&
            subJunction?.BILLING_SUBSCRIPTION_ID
          ) {
            billingData = {
          ...
          const cpSubscribeResponse = await createCPSubscription(
            req,
            api.apiReferenceID,
            cpAppID,
            policyDetails,
        ...
        for (const apiSubscription of apiSubscriptions) {
          ...
          const sharedKeyMapping = await adminDao.getApplicationAPIMapping(
            orgID,
            appID,
          ...
          const nonSharedKeyMapping = await adminDao.getApplicationAPIMapping(
            orgID,
            appID,
            apiSubscription.apiId,
            cpAppID,
        ...
        if (apiSubscriptions.length > 0) {
          logger.info(
            "Deleting app key mapping entries with no API ID reference",
            {
              orgId: req.params?.orgId,
              applicationId: req.params?.applicationId,
            },
        ...
        tokenDetails.additionalProperties = checkAdditionalValues(
          tokenDetails.additionalProperties,
        );

        if (
          tokenDetails.keyManager.startsWith(
            constants.KEY_MANAGERS.INTERNAL_KEY_MANAGER,
          ) ||
          tokenDetails.keyManager.startsWith(
            constants.KEY_MANAGERS.RESIDENT_KEY_MANAGER,
          ) ||
          tokenDetails.keyManager.startsWith(
            constants.KEY_MANAGERS.APP_DEV_STS_KEY_MANAGER,
          )
        ) {
          //generate oauth key
          ...
          if (!tokenDetails.keyType) {
            return util.handleError(
              ...
              new CustomError(
                400,
                constants.ERROR_CODE[400],
                "keyType is required in tokenDetails",
              ),
            );
          }
          responseData = await mapKeys(
            req,
            clientID,
            tokenDetails.keyManager,
            cpAppID,
            tokenDetails.keyType,
          );
        }
        // Add the appRefId to the response data
        responseData.appRefId = cpAppID;
        const cpApp = await invokeApiRequest(
          req,
          "GET",
          `${controlPlaneUrl}/applications/${cpAppID}`,
          {},
          {},
        );
...
async function mapKeys(req, clientID, keyManager, applicationId, keyType) {
  ...
  try {
    return await invokeApiRequest(
      req,
      "POST",
      `${controlPlaneUrl}/applications/${applicationId}/map-keys`,
      {},
      body,
    );
  } catch (error) {
...
async function generateOAuthKey(req, cpAppID, tokenDetails) {
  ...
  try {
    return await invokeApiRequest(
      req,
      "POST",
      `${controlPlaneUrl}/applications/${cpAppID}/generate-keys`,
      {},
      tokenDetails,
    );
  } catch (error) {
    try {
      if (error.statusCode && error.statusCode === 409) {
        logger.info(
          "OAuth key already exists in control plane, retrieving existing key",
          {
            cpAppID,
          },
        );
        const response = await invokeApiRequest(
          req,
          "GET",
          `${controlPlaneUrl}/applications/${cpAppID}/keys`,
          {},
        );
...
const createCPApplicationOnBehalfOfUser = async (
  cpApplicationName,
  owner,
  cpOrgId,
  patToken,
  ...
  try {
    ...
    const cpAppCreationResponse = await util.apiRequest(
      "POST",
      url,
      headers,
      {
    ...
    if (error.statusCode && error.statusCode === 409) {
      try {
        logger.info(
          "Application already exists in control plane, retrieving existing application",
          {
            orgId: cpOrgId,
            cpApplicationName,
          },
        );
        const cpAppResponse = await util.apiRequest(
          "GET",
          `${controlPlaneGwUrl}/applications?query=${cpApplicationName}`,
          headers,
          {},
          cpOrgId,
        );
...
const createCPSubscriptionOnBehalfOfUser = async (
  apiId,
  cpAppID,
  policyName,
  cpOrgId,
  ...
  try {
    ...
    const cpSubscribeResponse = await util.apiRequest(
      "POST",
      url,
      headers,
      requestBody,
    ...
    if (error.statusCode && error.statusCode === 409) {
      const response = await util.apiRequest(
        "GET",
        `${controlPlaneGwUrl}/subscriptions?apiId=${apiId}&applicationId=${cpAppID}`,
        headers,
        null,
        cpOrgId,
      );
...
const createAppKeyMappingOnBehalfOfUser = async (
  cpAppID,
  keymanager,
  clientId,
  keyType,
  ...
  try {
    ...
    if (error.statusCode && error.statusCode === 409) {
      const response = await util.apiRequest(
        "GET",
        `${controlPlaneGwUrl}/applications/${cpAppID}/oauth-keys`,
        headers,
        null,
        cpOrgId,
      );
      ...
      if (!response.data || !Array.isArray(response.data.list)) {
        throw new CustomError(
          ...
          "Invalid response structure from control plane",
        );
      }
      ...
      for (const keyMapping of response.data.list) {
        if (
          keyMapping.keyManager === keymanager &&
          keyMapping.keyType === keyType &&
          keyMapping.consumerKey === clientId
        ) {
...
const getAPIMKeyManagersBehalfOfUser = async (cpOrgId, patToken) => {
  ...
  const keymanagersResponse = await util.apiRequest(
    "GET",
    url,
    headers,
    null,
...
const createCPApplication = async (req, cpApplicationName) => {
  ...
  try {
    //create control plane application
    const cpAppCreationResponse = await invokeApiRequest(
      req,
      "POST",
    ...
    if (error.statusCode && error.statusCode === 409) {
      try {
        ...
        const cpAppResponse = await invokeApiRequest(
          req,
          "GET",
          `${controlPlaneUrl}/applications?query=${cpApplicationName}`,
          {},
          {},
        );
...
const createCPSubscription = async (
  req,
  apiId,
  cpAppID,
  policyDetails,
  ...
  logger.info("Creating control plane subscription", {
    apiId,
    cpAppID,
    policyDetails: policyDetails.dataValues
      ? policyDetails.dataValues.POLICY_NAME
      : policyDetails,
  ...
  try {
    const requestBody = {
      apiId: apiId,
      applicationId: cpAppID,
      throttlingPolicy: policyDetails.dataValues
        ? policyDetails.dataValues.POLICY_NAME
        : policyDetails,
    };
    ...
    if (billingData) {
      requestBody.billingMetadata = {
        ...
        userEmail: billingData.email || null,
      };
    }

    const cpSubscribeResponse = await invokeApiRequest(
      req,
      "POST",
      `${controlPlaneUrl}/subscriptions`,
      {},
    ...
    if (error.statusCode && error.statusCode === 409) {
      const response = await invokeApiRequest(
        req,
        "GET",
        `${controlPlaneUrl}/subscriptions?apiId=${apiId}&applicationId=${cpAppID}`,
        {},
      );
      return response.list[0];
    }
...
const unsubscribeAPI = async (req, res) => {
  ...
  try {
    ...
    try {
      const subscription = await adminDao.getSubscription(
        orgID,
        subscriptionID,
      );
      if (
        ...
        try {
            ...
            for (const dataValues of nonSharedToken) {
              if (dataValues.API_REF_ID === apiReferenceID) {
                await invokeApiRequest(
                  req,
                  "DELETE",
                  `${controlPlaneUrl}/subscriptions/${dataValues.SUBSCRIPTION_REF_ID}`,
                  {},
                  {},
                );
          ...
          try {
            if (error.statusCode && error.statusCode === 404) {
              logger.info(
                "Subscription not found in control plane, deleting from database",
                {
                  subscriptionId: req.params?.subscriptionId,
                },
              );
