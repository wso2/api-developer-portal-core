    ...
    try {
        ...
        await sequelize.transaction({
            ...
            try {
                ...
                let isShared = false;
                let applicationRefId = "";
                let apiRefId = "";
                let subRefId = "";

                if (sharedApp.length > 0) {
                    applicationRefId = sharedApp[0].dataValues.CP_APP_REF;
                    apiRefId = sharedApp[0].dataValues.API_REF_ID;
                    subRefId = sharedApp[0].dataValues.SUBSCRIPTION_REF_ID;
                    isShared = true;
                } else if (nonSharedApp.length > 0) {
                    applicationRefId = nonSharedApp[0].dataValues.CP_APP_REF;
                    apiRefId = nonSharedApp[0].dataValues.API_REF_ID;
                    subRefId = nonSharedApp[0].dataValues.SUBSCRIPTION_REF_ID;
                    isShared = false;
                } else {
                    return res.status(400).json({ message: "Application key mapping not found" });
                }

                const subscription = await adminDao.getAppApiSubscription(orgID, req.body.applicationID, req.body.apiId);
                
                const billingMetadata = subscription?.length > 0 && (subscription[0].BILLING_CUSTOMER_ID 
                    || subscription[0].BILLING_SUBSCRIPTION_ID)
                    ? {
                        billingCustomerId: subscription[0].BILLING_CUSTOMER_ID,
                        billingSubscriptionId: subscription[0].BILLING_SUBSCRIPTION_ID,
                    } : null;
                ...
                await handleSubscribe(orgID, req.body.applicationID, apiRefId, subRefId, response, isShared, t);
            ...
            try {
                ...
                if (app.length > 0) {
                    ...
                    if (appAPIMapping.length > 0) {
                        ...
                        const paymentStatus = subscription?.length > 0 && subscription[0].PAYMENT_STATUS;
