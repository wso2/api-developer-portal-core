...
Handlebars.registerHelper("filterByStatus", function (array, status) {
  ...
  return array.filter(
    (item) => item.status && item.status.toLowerCase() === statusLower,
  );
});
  ...
  try {
    ...
    const safe = context.map(function (s) {
      return {
        ...
        maskedToken: s.subscriptionToken
          ? "****" + String(s.subscriptionToken).slice(-4)
          : "****",
      };
    });
...
Handlebars.registerHelper("eq", function (a, b) {
  return (
    a === b ||
    (a != null && b != null && (a === b.toString() || a.toString() === b))
  );
});
...
Handlebars.registerHelper(
  "conditionalIf",
  function (condition, value1, value2) {
...
const strategy = new OAuth2Strategy(
  ...
  async (req, accessToken, refreshToken, params, profile, done) => {
    ...
    if (config.advanced.tokenExchanger?.enabled) {
      try {
        const exchangedToken = await util.tokenExchanger(
          accessToken,
          req.session.returnTo.split("/")[1],
        );
    ...
    const organizationID = decodedJWT[
      claimNames[constants.ROLES.ORGANIZATION_CLAIM]
    ]
      ? decodedJWT[config.orgIDClaim]
      : "";
    const roles = decodedJWT[claimNames[constants.ROLES.ROLE_CLAIM]]
      ? decodedJWT[config.roleClaim]
      : "";
    const groups = decodedJWT[claimNames[constants.ROLES.GROUP_CLAIM]]
      ? decodedJWT[config.groupsClaim]
      : "";
    let isAdmin,
      isSuperAdmin = false;
    if (
      roles.includes(constants.ROLES.SUPER_ADMIN) ||
    ...
    if (returnTo) {
      ...
      const endIndex =
        returnTo.indexOf("/", startIndex) !== -1
          ? returnTo.indexOf("/", startIndex)
          : returnTo.length;
      view = returnTo.substring(startIndex, endIndex);
    }
    let imageURL =
      "https://raw.githubusercontent.com/wso2/docs-bijira/refs/heads/main/en/devportal-theming/profile.svg";
...
passport.serializeUser((user, done) => {
  ...
  const profile = {
    ...
    [constants.ROLES.ORGANIZATION_CLAIM]:
      user[constants.ROLES.ORGANIZATION_CLAIM],
...
app.use(
  constants.ROUTE.TECHNICAL_STYLES,
  express.static(path.join(require.main.filename, "../styles")),
);
app.use(
  constants.ROUTE.TECHNICAL_SCRIPTS,
  express.static(path.join(require.main.filename, "../scripts")),
);
...
if (config.mode === constants.DEV_MODE) {
  app.use(
    constants.ROUTE.STYLES,
    express.static(path.join(process.cwd(), filePrefix + "styles")),
  );
  app.use(
    constants.ROUTE.IMAGES,
    express.static(path.join(process.cwd(), filePrefix + "images")),
  );
  app.use(
    constants.ROUTE.MOCK,
    express.static(path.join(process.cwd(), filePrefix + "mock")),
  );
  ...
  app.use(
    constants.ROUTE.STYLES,
    express.static(
      path.join(process.cwd(), "./src/defaultContent/" + "styles"),
    ),
  );
  app.use(
    constants.ROUTE.IMAGES,
    express.static(
      path.join(process.cwd(), "./src/defaultContent/" + "images"),
    ),
  );
...
app.use((err, req, res, next) => {
  ...
  if (err.status === 401) {
    ...
    templateContent.errorMessage =
      constants.ERROR_MESSAGE.COMMON_AUTH_ERROR_MESSAGE;
    html = util.renderTemplate(
      "../pages/error-page/page.hbs",
      "src/pages/error-layout/main.hbs",
      templateContent,
      true,
    );
  } else {
...
const logStartupInfo = () => {
  ...
  if (config.mode === constants.DEV_MODE) {
    logger.info(
      "⚠️  Since you are in DEV mode, ensure default content is available at configured pathToContent " +
  ...
  const visitUrl =
    config.baseUrl +
    (config.mode === constants.DEV_MODE
      ? "/views/default"
      : "/<organization>/views/default");
