...
const loadViewSettingsPage = async (req, res) => {
    ...
    try {
        if (config.mode === constants.DEV_MODE) {
            ...
            templateContent.llmsConfig = { aiEnabled: true, portalName: '', portalDescription: '', customInstructions: '' };
        } else {
            ...
            let llmsConfig = { aiEnabled: true, portalName: '', portalDescription: '', customInstructions: '' };
            if (configAsset) {
...
const getLlmsConfig = async (req, res) => {
    ...
    try {
        ...
        if (!asset) {
            return res.json({ aiEnabled: true, portalName: '', portalDescription: '', customInstructions: '' });
        }
...
const saveLlmsConfig = async (req, res) => {
    const { orgName, viewName } = req.params;
    const { aiEnabled, portalName, portalDescription, customInstructions } = req.body;
    try {
        const orgID = await adminDao.getOrgId(orgName);
        const content = Buffer.from(JSON.stringify({ aiEnabled, portalName, portalDescription, customInstructions }));
        const orgData = {
